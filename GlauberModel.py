...
while count_1 <= Y - 2013:
    try:
        ...
        print(str(
            Y - count_1) + "total cross section data is unavailable. The Particle Data Group website may not have the "
...
def distribute1D_test(x, prob, A_n, N):
    """Takes any numerical distribution probability, on
    the interval defined by the array x, and returns an
    array of N sampled values that are statistically the
    same as the input data."""
    y = prob * 4 * np.pi * x ** 2
    A = np.full((N, len(x)), np.divide(np.cumsum(y), np.cumsum(y)[-1]))
    z = np.random.rand(N, A_n)
    B = np.array(list(map(np.searchsorted, A, z)))
    return x[B]
...
def Collider(N, Particle1, A1, Particle2, A2, model1, model2, Energy, bRange=1.6, Range=2, Bins=100):
    ...
    b = np.random.choice(b_range, N, p=b_prob)
    ...
    # TODO Let's do some Python broadcasting instead of running a loop N times, shall we?

    # Arrange the nucleons within the nucleus.
    # The distribute1D function is in spherical coordinates which we then transform into Cartesian coordinates.
    Nucleus1 = np.zeros((7, N, A1))
    Nucleus1[0] = distribute1D_test(r1, NCD(Particle1, model1, Range, Bins), A1, N)
    Nucleus1[1] = np.arccos(2 * np.random.random_sample((N, A1)) - 1)
    Nucleus1[2] = np.multiply(2 * np.pi, np.random.random_sample((N, A1)))
    ...
    Nucleus2 = np.zeros((7, N, A2))
    Nucleus2[0] = distribute1D_test(r2, NCD(Particle2, model2, Range, Bins), A2, N)
    Nucleus2[1] = np.arccos(2 * np.random.random_sample((N, A2)) - 1)
    Nucleus2[2] = np.multiply(2 * np.pi, np.random.random_sample((N, A2)))
    ...
    for L in range(N):
        ...
        Nucleus1[:, 1] = np.arccos(np.multiply(2, np.random.random_sample(A1))-1)
        Nucleus1[:, 2] = np.multiply(2 * np.pi, np.random.random_sample(A1))
        Nucleus1[:, 3] = np.multiply(np.multiply(Nucleus1[:, 0], np.sin(Nucleus1[:, 1])), np.cos(Nucleus1[:, 2]))
        Nucleus1[:, 4] = np.multiply(np.multiply(Nucleus1[:, 0], np.sin(Nucleus1[:, 1])), np.sin(Nucleus1[:, 2]))
        Nucleus1[:, 5] = np.multiply(Nucleus1[:, 0], np.cos(Nucleus1[:, 1]))

        Nucleus2[:, 1] = np.arccos(2*np.random.random_sample(A2)-1)
        Nucleus2[:, 2] = 2 * np.pi * np.random.random_sample(A2)
        Nucleus2[:, 3] = np.multiply(np.multiply(Nucleus2[:, 0], np.sin(Nucleus2[:, 1])), np.cos(Nucleus2[:, 2]))
        Nucleus2[:, 4] = np.multiply(np.multiply(Nucleus2[:, 0], np.sin(Nucleus2[:, 1])), np.sin(Nucleus2[:, 2]))
        Nucleus2[:, 5] = np.multiply(Nucleus2[:, 0], np.cos(Nucleus2[:, 1]))
        ...
        for p1 in range(A1):  # Loops through all nucleons in Nucleus 1
            ...
            for p2 in range(A2):  # Loops through all nucleons in Nucleus 2
                if np.sqrt((Nucleus1[p1, 3] - Nucleus2[p2, 3]) ** 2 + (
                        b[L] + Nucleus2[p2, 4] - Nucleus1[p1, 4]) ** 2) <= Maxr:
                    Ncoll[L] += 1
